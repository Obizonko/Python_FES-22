...
class EmployedBee:
    ...
    def fly(self, source):
        ...
        return f'{self.bee_name} is flying to  {source.location}'

    def _gather(self):
        self.taken_food += (self.current_source.take_food(self.food_amount) if self.current_source is not None else 0)
        log = None
        if self.current_source is not None:
            log = f'{self.bee_name} is taking {self.taken_food} food dressers from {self.current_source.location}'
