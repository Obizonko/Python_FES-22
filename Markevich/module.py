...
class Game:
    ...
    def placePieces(self):
        ...
        white_pieces = random.randint(2, 5)  # choose 2 to 5 pieces for white
        black_pieces = random.randint(2, 5)  # choose 2 to 5 pieces for black

        # Place placers for both sides
        for i in range(0, 8):
            # Place the white placer
            if i == 0:
                self.gameboard[(i, 0)] = placers[i](WHITE, uniDict[WHITE][placers[i]])
                white_pieces -= 1
            else:
                # Try placing the white placer randomly until it is valid
                while white_pieces > 0:
                    row = random.randint(0, 7)
                    col = random.randint(0, 7)
                    if (row, col) not in self.gameboard.keys():
                        self.gameboard[(row, col)] = placers[i](WHITE, uniDict[WHITE][placers[i]])
                        white_pieces -= 1
                        break

            # Place the black placer
            if i == 0:
                self.gameboard[((7 - i), 7)] = placers[i](BLACK, uniDict[BLACK][placers[i]])
                black_pieces -= 1
            else:
                # Try placing the black placer randomly until it is valid
                while black_pieces > 0:
                    row = random.randint(0, 7)
                    col = random.randint(0, 7)
                    if (row, col) not in self.gameboard.keys():
                        self.gameboard[((7 - i), col)] = placers[i](BLACK, uniDict[BLACK][placers[i]])
                        black_pieces -= 1
                        break
    ...
    def isCheck(self):
        ...
        # white
        if self.canSeeKing(kingDict[WHITE], [piece[0] for piece in pieceDict[BLACK]]):
            self.message = "White player is in check"
        if self.canSeeKing(kingDict[BLACK], [piece[0] for piece in pieceDict[WHITE]]):
